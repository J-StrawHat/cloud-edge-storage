 ...
 * @brief implement the interface of the base of cloud 
 * @version 0.1
 * @date 2023-05-05
 * 
...
#include "cloudBase.h" // #include "../../include/enclaveBase.h"
...
CloudBase::CloudBase() {
    ...
    Enclave::Logging(myName_.c_str(), "init the CloudBase.\n");
}
 ...
 */
bool CloudBase::IsEndOfSegment(uint32_t chunkHashVal, uint32_t chunkSize, 
    Segment_t* segment) {
 ...
 */
uint32_t CloudBase::ConvertHashToValue(const uint8_t* inputHash) {
    uint32_t hashVal = 0;
 ...
 */
void CloudBase::UpdateFileRecipe(string& chunkAddrStr, Recipe_t* inRecipe,
    UpOutSGX_t* upOutSGX) {
 ...
 * @param upOutSGX the upload out-enclave var
 */
void CloudBase::ProcessUniqueChunk(RecipeEntry_t* chunkAddr, uint8_t* chunkBuffer, 
 ...
 * @return false fail
 */
bool CloudBase::UpdateIndexStore(const string& key, const char* buffer, 
    size_t bufferSize) {
    bool status;
 ...
 * @return false 
 */
bool CloudBase::ReadIndexStore(const string& key, string& value,
    UpOutSGX_t* upOutSGX) {
    bool status;
 ...
 */
double CloudBase::GetTimeDiffer(uint64_t sTime, uint64_t eTime) {
 ...
 */
void CloudBase::ResetCurrentSegment(EnclaveClient* sgxClient) {
