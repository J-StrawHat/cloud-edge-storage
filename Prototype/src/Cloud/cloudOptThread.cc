...
#include "cloudOptThead.h"// #include "../../include/serverOptThead.h"
 ...
 */
CloudOptThead::CloudOptThead(SSLConnection* dataSecureChannel, 
    ...
    tool::Logging(myName_.c_str(), "init the CloudOptThead.\n");
}   
...
CloudOptThead::~CloudOptThead() {
    ...
    fprintf(stderr, "========CloudOptThead Info========\n");
 ...
 */
void CloudOptThead::Run(SSL* clientSSL) {
    boost::thread* thTmp;
...
#if (ENABLE_SGX_RA == 1)
    // check whether do remote attestation(RA)
    ...
    } //"%02x"表示以两位十六进制形式格式化无符号字符，sprintf()将dataBuffer的字节转换为十六进制字符串并存至fileHashBuf中
    string fileName;
    fileName.assign(fileHashBuf, CHUNK_HASH_SIZE * 2);
    string recipePath = config.GetRecipeRootPath() +
        fileName + config.GetRecipeSuffix(); // filename -> file recipe [path]
    if (!this->CheckFileStatus(recipePath, optType)) { // 服务器文件不存在
        recvBuf.header->messageType = SERVER_FILE_NON_EXIST; // 试图从服务器恢复文件，但是filerecipe不存在
        if (!dataSecureChannel_->SendData(clientSSL, recvBuf.sendBuffer,
    ...
    ClientVar* outClient;
    switch (optType) {
        case UPLOAD_OPT: { // TODO cloud : server 上传 fp，首先验证 fp 对应的 chunk 是否存在
            ...
            // bind()由将dataReceiverObj_去执行Run函数，参数分别为 outClient, enclaveInfo
 ...
 * @return false fail
 */
bool CloudOptThead::CheckFileStatus(string& fullRecipePath, int optType) {
