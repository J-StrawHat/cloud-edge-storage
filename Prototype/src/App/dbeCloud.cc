...
int main(int argc, char* argv[]) {
    ...
    sigaction(SIGPIPE, &sigIntHandler, 0); // 信号ID, SIGPIPE信号通常在向一个已经被捕获并关闭的socket或者管道上写数据时会触发

    sigIntHandler.sa_handler = CTRLC; // 信号处理函数
    sigaction(SIGKILL, &sigIntHandler, 0); // Ctrl + C组合键产生的，常用来通知进程需要终止运行并清理资源
    sigaction(SIGINT, &sigIntHandler, 0); // 系统管理员强制终止某个进程使用的，该信号是不能被阻塞、处理或者忽略的
