...
CloudOptThread* cloudThreadObj;

void Usage() {
    fprintf(stderr, "./DEBECloud -m [IndexType]\n"
...
void CTRLC(int s) {
    tool::Logging(myName.c_str(), "terminate the cloud with ctrl+c interrupt\n");
    // ------ clean up ------
    for (auto it : thList) {
        it->join();
    }
...
int main(int argc, char* argv[]) {
    ...
    /**
     * |---------------------------------------|
     * |Finish the initialization of the cloud |
     * |---------------------------------------|
     */
